...
interface IncomeBudgetChartProps {
  incomes: Income[];
  selectedMonth: string; // "YYYY-MM" or "all"
}

// Color thresholds (same as BudgetChart):
// green  = actual < planned (under budget — for income this means earning less than planned)
// orange = actual within ±10% of planned
// red    = actual > 110% of planned (over budget — for income this means earning more, which is good)
// Note: We keep the same visual semantics for consistency with expenses chart
function getActualColor(actual: number, planned: number): { bg: string; border: string } {
  if (planned === 0) {
    return actual > 0
      ? { bg: "#4BA56ACC", border: "#4BA56A" }  // green — unplanned income is good
      : { bg: "#4BA56ACC", border: "#4BA56A" };
  }
  const ratio = actual / planned;
  if (ratio > 1.1) {
    return { bg: "#4BA56ACC", border: "#4BA56A" }; // green — exceeding income target
  }
  if (ratio >= 0.9) {
    return { bg: "#F59E0BCC", border: "#D97706" }; // orange — close to target
  }
  // Under target
  return { bg: "#D65A56CC", border: "#D65A56" }; // red — below income target
}

export default function IncomeBudgetChart({
  incomes,
  selectedMonth,
}: IncomeBudgetChartProps) {
  ...
  useEffect(() => {
    const fetchBudget = async () => {
      ...
      try {
        ...
        logger.error("[IncomeBudgetChart] Failed to load budget entries", error);
        setBudgetEntries([]);
      } finally {
        setLoading(false);
      }
  ...
  // Check if an income is active in the selected month
  const isActiveInMonth = (income: Income): boolean => {
  ...
  const comparisonData = useMemo(() => {
    // Planned: group income budget entries by category
    ...
    // Actual: group actual incomes by category for this month
    const actual = new Map<string, number>();
    for (const income of incomes) {
      if (!isActiveInMonth(income)) continue;
      actual.set(
        income.category,
        (actual.get(income.category) ?? 0) + income.amount,
      );
    }
    ...
    return rows;
  }, [budgetEntries, incomes, selectedMonth, year, monthNum]);
  ...
  const categoryLabels = comparisonData.map((row) =>
    intl.formatMessage({
      id: `income.categories.${row.category}`,
  ...
  return (
    <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50/30 via-white to-white p-6 shadow-sm space-y-3">
      <div>
